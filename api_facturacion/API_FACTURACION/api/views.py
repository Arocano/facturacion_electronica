...
from .models import Usuario, Empresa, Iva, Detalle_empresa_iva, Producto, Detalle_empresa_producto, Cliente, Detalle_empresa_cliente, Licencia, Factura, Detalle_factura, Forma_pago, Documento,Email
...
class AbrirFacturaView(View):
    ...
    def post(self, request):
        try:
            ...
            forma_pago=jsonData['id_forma_pago_per']
...
class AgregarProductoView(View):
    ...
    def post(self, request):
        try:
            jsonData = json.loads(request.body)
            factura = Factura.objects.get(id_factura=jsonData['id_factura_per'], estado='abierta')
            if Factura.objects.filter(id_factura=factura.id_factura, estado='abierta').exists() :
                #and Producto.objects.filter(id_producto=jsonData['id_producto_per']).exists()
                # producto = Producto.objects.filter(
                #     id_producto=jsonData['id_producto_per']).values().first()
                iva = Iva.objects.get(id_iva=jsonData['id_iva'])
                detalle_factura = Detalle_factura.objects.create(
                    id_factura_per=factura.id_factura,
                    id_producto_per=jsonData['id_producto_per'],
                    cantidad=jsonData['cantidad'],
                    precio_unitario=jsonData['precio'],
                    producto=jsonData['nombre'],
                    subtotal_producto=float(
                        jsonData['cantidad']) * float(jsonData['precio']),
                    total_iva=float(jsonData['cantidad']) *
                    float(jsonData['precio']) * float(iva.iva)
                )
...
class CerrarFacturaView(View):
        ...
        try:
            ...
            if factura is not None:
                ...
                factura.clave_acceso = self.generar_clave_acceso(factura.fecha.strftime(
                    '%d%m%Y'), '01', empresa.ruc, '1', serial, str(factura.numero_factura).zfill(9), '71011173', '1')
                subtotal = 0
                total_iva = 0
                total = 0
                ...
                iva=jsonData['id_iva']
                formapago=jsonData['id_forma_pago_per']
                ride = self.generar_pdf_factura(factura, Detalle_factura.objects.filter(id_factura_per=factura.id_factura),cliente)
                xml_data = self.generar_xml(factura, Detalle_factura.objects.filter(id_factura_per=factura.id_factura), empresa,cliente,iva,formapago)
                signed_xml = self.firmar_xml(xml_data, factura)
                validar = self.validar_xml(signed_xml)
                if validar.estado != "RECIBIDA":
                    raise Exception(f"Proceso detenido. Estado: {validar.estado}")
            ...
            #print(f"Ocurrió un error: {e}")
            print(validar.comprobantes.comprobante[0].mensajes.mensaje[0].mensaje)
            datos = {"Factura":[validar.estado ,validar.comprobantes.comprobante[0].mensajes.mensaje[0].mensaje]}
            return JsonResponse(datos, status=400)
        return JsonResponse(datos)


    def generar_pdf_factura(self, factura, detalles_factura,cliente):
    ...
    def generar_xml(self, factura_recibida, detalles_factura, empresa,cliente,iva,formapago):
        ...
        obligado_contabilidad.text = 'SI' if (empresa.lleva_contabilidad) else 'NO'
        tipo_identificacion_comprador = ET.SubElement(info_factura, 'tipoIdentificacionComprador')
        tipo_identificacion_comprador.text = '07' if cliente['numero_identificacion'] == '9999999999999' else ('04' if len(cliente['numero_identificacion']) == 13 and cliente['numero_identificacion'][-3:] == '001' else '05')
        ...
        identificacion_comprador.text = cliente['numero_identificacion']
        ...
        if iva == 1:
            ...
            codigo_porcentaje.text = '0'
        elif iva == 3:
            codigo_porcentaje.text = '3'
        elif iva == 4:
            codigo_porcentaje.text = '6'
        elif iva == 5:
            codigo_porcentaje.text = '7'
        else:
            codigo_porcentaje.text = '0' 
        ...
        for detalle in detalles_factura:
            ...
            descuento.text = '0.00'
            precio_total_sin_impuesto = ET.SubElement(detalle_element, 'precioTotalSinImpuesto')
            precio_total_sin_impuesto.text = str(detalle.subtotal_producto)

            detalles_adicionales = ET.SubElement(detalle_element, 'detallesAdicionales')
            det_adicional = ET.SubElement(detalles_adicionales, 'detAdicional')
            det_adicional.set('nombre', 'informacionAdicional')
            det_adicional.set('valor', str(detalle.producto))
    ...
    def validar_xml(self, signed_xml):
        ...
        # else:
        #     raise Exception(f"Proceso detenido. Estado: {estado}")
    ...
    def enviar_comprobante_correo(self, documento, factura):
        try:
            ...
            if cliente.correo is not None and cliente.numero_identificacion != '9999999999999':
                ...
                message["Subject"] = "Solinfo: Tu factura está disponible"
                message["From"] = email.correo
                ...
                smtp_port = email.puerto
                email_address = email.correo
    ...
    def get(self, request, id_empresa=None):
        if id_empresa is not None:
            ...
            subquery_facturas = Factura.objects.filter(
                id_usuario_per__in=Usuario.objects.filter(
